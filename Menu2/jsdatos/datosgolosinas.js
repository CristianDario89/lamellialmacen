var alfajores = [ {
    id:"1",
    marca: "alfajor <strong>milka</strong>",
    descripcion: "dulce de leche - Mousse",
    precio: "$70",
    cantidad:"",
    seccion:"",
    proveedor:""
},
{
    id:"2",
    marca: "<strong>coffler </strong>block",
    descripcion: "",
    cantidad:"",
    precio: "$70",
    cantidad:"",
    seccion:"",
    proveedor:"" 
},
{
    id:"3",
    marca: "<strong>aguila </strong>minitorta",
    descripcion: "",
    precio: "$60",
    cantidad:"",
    seccion:"",
    proveedor:""
},
{
    id:"4",
    marca: "alfajor <strong>tri-shot</strong>",
    descripcion: "",
    cantidad:"",
    precio: "$70",
    cantidad:"",
    seccion:"",
    proveedor:"" 
},
{
    id:"5",
    marca: "alfajor <strong>bon-o-bon</strong>",
    descripcion: "blanco - negro",
    cantidad:"",
    precio: "$60",
    cantidad:"",
    seccion:"",
    proveedor:"" 
},
                 {
    id:"6",
    marca: "alfajor <strong>bon-o-bon</strong>",
    descripcion: "triple",
    cantidad:"",
    precio: "60",
    cantidad:"",
    seccion:"",
    proveedor:"" 
},
{
    id:"7",
    marca: "alfajor <strong>jorgelin</strong>",
    descripcion: "blanco - negro",
    cantidad:"",
    precio: "$50",
    cantidad:"",
    seccion:"",
    proveedor:"" 
},
{
    id:"8",
    marca: "alfajor <strong>pepitos</strong>",
    descripcion: "",
    cantidad:"",
    precio: "$50",
    cantidad:"",
    seccion:"",
    proveedor:"" 
},
{
    id:"9",
    marca: "alfajor <strong>tifu</strong>",
    descripcion: "",
    cantidad:"",
    precio: "$70",
    cantidad:"",
    seccion:"",
    proveedor:"" 
},
{
    id:"10",
    marca: "<strong>guaymallen</strong>",
    descripcion: "simple chocolate - dulce de leche",
    cantidad:"",
    precio: "$30",
    cantidad:"",
    seccion:"",
    proveedor:"" 
},
{
    id:"11",
    marca: "alfajor <strong>jorgito</strong>",
    descripcion: "",
    cantidad:"",
    precio: "$40",
    cantidad:"",
    seccion:"",
    proveedor:"" 
},
{
    id:"12",
    marca: "alfajor <strong>bombom</strong>",
    descripcion: "",
    cantidad:"",
    precio: "$60",
    cantidad:"",
    seccion:"",
    proveedor:"" 
},
{
    id:"13",
    marca: "alfajor <strong>bombom</strong> simple",
    descripcion: "blanco y negro",
    cantidad:"",
    precio: "$45",
    cantidad:"",
    seccion:"",
    img:"" 
}
];
/*----------------------------
var chocolates = [ {
    id:"1",
    marca: "<strong>milka </strong> dulce de leche",
    descripcion: "",
    precio: "$70",
    cantidad:"",
    seccion:"",
    proveedor:""
},
{
    id:"2",
    marca: "<strong>milka leger </strong> combinado",
    descripcion: "",
    cantidad:"",
    precio: "$70",
    cantidad:"",
    seccion:"",
    proveedor:"" 
},
{
    id:"3",
    marca: "<strong>milka leger </strong> almendras",
    descripcion: "",
    precio: "$60",
    cantidad:"",
    seccion:"",
    proveedor:""
},
{
    id:"4",
    marca: "<strong>milka leger </strong> leche",
    descripcion: "",
    cantidad:"",
    precio: "$70",
    cantidad:"",
    seccion:"",
    proveedor:"" 
},
{
    id:"5",
    marca: "<strong>milka </strong> bis",
    descripcion: "",
    cantidad:"",
    precio: "$60",
    cantidad:"",
    seccion:"",
    proveedor:"" 
},
                 {
    id:"6",
    marca: "<strong>milka </strong> oreo",
    descripcion: "",
    cantidad:"",
    precio: "$310",
    cantidad:"",
    seccion:"",
    proveedor:"" 
},
{
    id:"7",
    marca: "<strong>milka </strong> blanco - negro",
    descripcion: "",
    cantidad:"",
    precio: "$50",
    cantidad:"",
    seccion:"",
    proveedor:"" 
}
];
/*--------------------------*/
var chocolates = [ {
    id:"1",
    marca: "<strong>milka </strong> dulce de leche",
    descripcion: "",
    precio: "$70",
    cantidad:"",
    seccion:"",
    img:""
},
{
    id:"2",
    marca: "<strong>milka leger </strong> combinado",
    descripcion: "",
    cantidad:"",
    precio: "$70",
    cantidad:"",
    seccion:"",
    img:"images/golo/gol9.jpg" 
},
{
    id:"3",
    marca: "<strong>milka leger </strong> almendras",
    descripcion: "",
    precio: "$60",
    cantidad:"",
    seccion:"",
    img:"images/golo/ggol4.png"
},
{
    id:"4",
    marca: "<strong>milka leger </strong> leche",
    descripcion: "",
    cantidad:"",
    precio: "$70",
    cantidad:"",
    seccion:"",
    img:"images/golo/ll.png" 
},
{
    id:"5",
    marca: "<strong>milka </strong> bis",
    descripcion: "",
    cantidad:"",
    precio: "$60",
    cantidad:"",
    seccion:"",
    img:"images/golo/gol7.jpg" 
},
{
    id:"6",
    marca: "<strong>milka </strong> oreo",
    descripcion: "",
    cantidad:"",
    precio: "$310",
    cantidad:"",
    seccion:"",
    img:"" 
},
{
    id:"7",
    marca: "<strong>milka </strong> blanco - negro",
    descripcion: "",
    cantidad:"",
    precio: "$50",
    cantidad:"",
    seccion:"",
    img:"" 
},
{
    id:"8",
    marca: "<strong>Bon o bom</strong>",
    descripcion: "dulce de leche - blanco",
    cantidad:"",
    precio: "$16",
    cantidad:"",
    seccion:"",
    img:"images/golo/ggol6.jpg" 
},
{
    id:"9",
    marca: "<strong>rodhesia </strong> ",
    descripcion: "",
    cantidad:"",
    precio: "$30",
    cantidad:"",
    seccion:"",
    img:"images/golo/gol1.jpg" 
},
                 {
    id:"10",
    marca: "<strong>tita </strong> ",
    descripcion: "",
    cantidad:"",
    precio: "$30",
    cantidad:"",
    seccion:"",
    img:"images/golo/gol2.jpg" 
},
{
    id:"11",
    marca: "<strong>tofi</strong>",
    descripcion: "",
    cantidad:"",
    precio: "$40",
    cantidad:"",
    seccion:"",
    img:"images/golo/gol4.jpg" 
},
{
    id:"12",
    marca: "<strong>Kinder</strong> ",
    descripcion: "",
    cantidad:"",
    precio: "$80",
    cantidad:"",
    seccion:"",
    img:"" 
},
 {
    id:"13",
    marca: "<strong>Ferrero roger </strong> caja",
    descripcion: "",
    cantidad:"",
    precio: "$310",
    cantidad:"",
    seccion:"",
    img:"" 
},
 {
    id:"14",
    marca: "<strong>rocklets </strong> 20g",
    descripcion: "",
    cantidad:"",
    precio: "$22",
    cantidad:"",
    seccion:"",
    img:"" 
},
 {
    id:"16",
    marca: "<strong>cardbury </strong> almendras",
    descripcion: "",
    cantidad:"",
    precio: "$50",
    cantidad:"",
    seccion:"",
    img:"" 
},
 {
    id:"17",
    marca: "<strong>cofler block </strong> 180g",
    descripcion: "",
    cantidad:"",
    precio: "$215",
    cantidad:"",
    seccion:"",
    img:"images/golo/gol8.jpg" 
},
 {
    id:"18",
    marca: "<strong>cofler block </strong> 120g",
    descripcion: "",
    cantidad:"",
    precio: "$120",
    cantidad:"",
    seccion:"",
    img:"" 
} ,
 {
    id:"19",
    marca: "<strong>cofler block </strong> 38g",
    descripcion: "",
    cantidad:"",
    precio: "$50",
    cantidad:"",
    seccion:"",
    img:"" 
} 
];
/*--------------------*/
var dulces1 = [ {
    id:"1",
    marca: "<strong>tictac </strong>",
    descripcion: "",
    precio: "$30",
    cantidad:"",
    seccion:"",
    img:""
},
{
    id:"2",
    marca: "<strong>go go </strong> chicle",
    descripcion: "$10 c/u",
    cantidad:"",
    precio: "$48",
    cantidad:"",
    seccion:"",
    img:"" 
},
{
    id:"3",
    marca: "<strong>chupetin </strong> comun",
    descripcion: "",
    precio: "$5",
    cantidad:"",
    seccion:"",
    img:""
},
{
    id:"4",
    marca: "<strong>push posh</strong>",
    descripcion: "",
    cantidad:"",
    precio: "$35",
    cantidad:"",
    seccion:"",
    img:"" 
},
{
    id:"5",
    marca: "<strong>mogul </strong> viborita acida",
    descripcion: "",
    precio: "$25",
    cantidad:"",
    seccion:"",
    img:""
},
{
    id:"6",
    marca: "<strong>biyu</strong> pastillas",
    descripcion: "",
    cantidad:"",
    precio: "$40",
    cantidad:"",
    seccion:"",
    img:"" 
},
{
    id:"7",
    marca: "<strong>chupetin </strong> comun",
    descripcion: "",
    precio: "$5",
    cantidad:"",
    seccion:"",
    img:""
},
{
    id:"8",
    marca: "<strong>mushy</strong> turron",
    descripcion: "",
    cantidad:"",
    precio: "$15",
    cantidad:"",
    seccion:"",
    img:"" 
},
{
    id:"9",
    marca: "<strong>toplain </strong> seven",
    descripcion: "negro - violeta - naranja",
    precio: "$38",
    cantidad:"",
    seccion:"",
    img:""
},
{
    id:"10",
    marca: "<strong>beldent</strong>",
    descripcion: "m - mf -f",
    cantidad:"",
    precio: "$30",
    cantidad:"",
    seccion:"",
    img:"" 
},
{
    id:"11",
    marca: "<strong>yipi</strong> pastillas",
    descripcion: "",
    precio: "$5",
    cantidad:"",
    seccion:"",
    img:""
},
{
    id:"12",
    marca: "<strong>push posh</strong>",
    descripcion: "",
    cantidad:"",
    precio: "$10",
    cantidad:"",
    seccion:"",
    img:"" 
}
];
/*--------------------------------*/
var galletitas = [ {
    id:"",
    secciones:["budines","facturas","dulces","saladas"],
    descripcion: "",
    proveedores:["budines","facturas","dulces","saladas"]   
} 
] 

/*--------------------------------*/
var proveedores = [ {
    id:"",
    nombre:"",
    telefono: "",
    direccion:"",
    productos:["budines","facturas","dulces","saladas"]   
} 
] 
/*-------------Card Overlay---------*/
var over2 = function(atr,one,two, three) {
	    var contenido = document.querySelector(atr)
	        contenido.innerHTML = `
      <span class="clearfix">
                        <a class="menu-title" href="#" data-meal-img="assets/img/restaurant/rib.jpg">${one}</a>
                        <span style="left: 166px; right: 44px;" class="menu-line"></span>
            <span class="menu-price">${two}</span>
            </span>
            <span class="menu-subtitle">${three}</span>
   `
    }     


over2("#p1", alfajores[0].marca,alfajores[0].precio,alfajores[0].descripcion);
over2("#p2", alfajores[1].marca,alfajores[1].precio,alfajores[1].descripcion);
over2("#p3", alfajores[2].marca,alfajores[2].precio,alfajores[2].descripcion);
over2("#p4", alfajores[3].marca,alfajores[3].precio,alfajores[3].descripcion);
over2("#p5", alfajores[4].marca,alfajores[4].precio,alfajores[4].descripcion);
over2("#p6", alfajores[5].marca,alfajores[5].precio,alfajores[5].descripcion);
over2("#p7", alfajores[6].marca,alfajores[6].precio,alfajores[6].descripcion);
over2("#p8", alfajores[7].marca,alfajores[7].precio,alfajores[7].descripcion);
over2("#p9", alfajores[8].marca,alfajores[8].precio,alfajores[8].descripcion);
over2("#p10", alfajores[9].marca,alfajores[9].precio,alfajores[9].descripcion);

over2("#pp1", alfajores[10].marca,alfajores[10].precio,alfajores[10].descripcion);
over2("#pp2", alfajores[11].marca,alfajores[11].precio,alfajores[11].descripcion);
over2("#pp3", alfajores[12].marca,alfajores[12].precio,alfajores[12].descripcion);


over2("#p11", chocolates[0].marca,chocolates[0].precio,chocolates[0].descripcion);
over2("#p22", chocolates[1].marca,chocolates[1].precio,chocolates[1].descripcion);
over2("#p33", chocolates[2].marca,chocolates[2].precio,chocolates[2].descripcion);
over2("#p44", chocolates[3].marca,chocolates[3].precio,chocolates[3].descripcion);
over2("#p55", chocolates[4].marca,chocolates[4].precio,chocolates[4].descripcion);
over2("#p66", chocolates[5].marca,chocolates[5].precio,chocolates[5].descripcion);
over2("#p77", chocolates[6].marca,chocolates[6].precio,chocolates[6].descripcion);


over2("#pc1", chocolates[7].marca,chocolates[7].precio,chocolates[7].descripcion);
over2("#pc2", chocolates[8].marca,chocolates[8].precio,chocolates[8].descripcion);
over2("#pc3", chocolates[9].marca,chocolates[9].precio,chocolates[9].descripcion);
over2("#pc4", chocolates[10].marca,chocolates[10].precio,chocolates[10].descripcion);
over2("#pc5", chocolates[11].marca,chocolates[11].precio,chocolates[11].descripcion);
over2("#pc6", chocolates[12].marca,chocolates[12].precio,chocolates[12].descripcion);
over2("#pc7", chocolates[13].marca,chocolates[13].precio,chocolates[13].descripcion);

over2("#pc8", chocolates[0].marca,chocolates[0].precio,chocolates[0].descripcion);
over2("#pc9", chocolates[1].marca,chocolates[1].precio,chocolates[1].descripcion);
over2("#pc10", chocolates[2].marca,chocolates[2].precio,chocolates[2].descripcion);
over2("#pc11", chocolates[3].marca,chocolates[3].precio,chocolates[3].descripcion);
over2("#pc12", chocolates[4].marca,chocolates[4].precio,chocolates[4].descripcion); 
/*--------------------*/

over2("#pd1", dulces1[0].marca,dulces1[0].precio,dulces1[0].descripcion);
over2("#pd2", dulces1[1].marca,dulces1[1].precio,dulces1[1].descripcion);
over2("#pd3", dulces1[2].marca,dulces1[2].precio,dulces1[2].descripcion);
over2("#pd4", dulces1[3].marca,dulces1[3].precio,dulces1[3].descripcion);
over2("#pd5", dulces1[4].marca,dulces1[4].precio,dulces1[4].descripcion);
over2("#pd6", dulces1[5].marca,dulces1[5].precio,dulces1[5].descripcion);
over2("#pd7", dulces1[6].marca,dulces1[6].precio,dulces1[6].descripcion);
over2("#pd8", dulces1[7].marca,dulces1[7].precio,dulces1[7].descripcion);
over2("#pd9", dulces1[8].marca,dulces1[8].precio,dulces1[8].descripcion);
over2("#pd10", dulces1[9].marca,dulces1[9].precio,dulces1[9].descripcion);




 